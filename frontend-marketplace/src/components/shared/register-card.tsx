...
export function RegisterCard({ onSuccess }: RegisterCardProps) {
  ...
  const handleSuccess = (data: RegisterFormData) => {
    if (onSuccess) {
      onSuccess(data);
    } else {
      // Flujo visual: Tras confirmación exitosa de registro, redirige al login tras 1.5s
      setTimeout(() => {
        router.push('/login');
      }, 1500);
    }
  ...
  return (
    <div className="w-full max-w-5xl bg-surface-main rounded-3xl shadow-xl border border-border-base overflow-hidden grid grid-cols-1 lg:grid-cols-12 animate-in fade-in zoom-in-95 duration-200">
      {/* Panel Izquierdo: Branding Allin1 (Oculto en móvil para máxima usabilidad) */}
      <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-10 lg:p-12 bg-linear-to-br from-[#0a5c36] via-[#0d684a] to-[#064326] text-white select-none">
        <div>
        ...
        <div className="space-y-4 my-auto py-12">
          ...
          <p className="text-emerald-100/90 text-sm lg:text-base leading-relaxed">
            Regístrate para acceder a todos los servicios, conectar con ofertas y demandas en tiempo
            real, y gestionar compras y envíos desde un solo lugar.
          </p>
        </div>
      ...
      <div className="lg:col-span-7 p-6 sm:p-10 lg:p-12 flex flex-col justify-center">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <button
            type="button"
            className="h-10 px-3 rounded-xl border border-border-base bg-surface-main text-content-main text-xs sm:text-sm font-semibold hover:bg-surface-base hover:border-border-base/80 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs"
            onClick={() => alert('El registro con GitHub estará disponible próximamente.')}
          >
        ...
        {/* Pie: Enlace a Login */}
        <div className="mt-5 text-center text-xs text-content-muted">
